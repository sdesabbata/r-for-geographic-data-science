...
read_csv(
  ...
  write_csv(
      paste0(
        Sys.getenv("RGDS_HOME"), 
        "/data/", 
        "2011_OAC_Leicester_StudentsAroundCampus.csv"
      )
  )
```
...
`r ''````{r libraries, warning=FALSE, message=FALSE}
library(tidyverse)
library(knitr)
`r ''````
...
Again, as mentioned above, don't worry too much about the details of the code right now. We will get to those in the coming weeks. Focus on understanding and embracing the **reproducible data science** workflow. `r emo::ji("smiling face with smiling eyes")`
