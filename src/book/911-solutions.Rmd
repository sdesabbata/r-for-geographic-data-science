...
The test is significant *F*(`r log10_u020_supgrpname_aov[[1]] %$% Df[1] %>% round(digits = 2)`, `r log10_u020_supgrpname_aov[[1]] %$% Df[2] %>% round(digits = 2)`) = `r log10_u020_supgrpname_aov[[1]][1, 4] %>% round(digits = 2)` with *p* < .01, thus we can say that the distribution of `u020` across the 2011OAC supergroups, excluding *Hard-Pressed Living*, are different.
...
The test is significant (*p* < .01), thus we can say that there is a relationship, which is however extremely weak. The value for tau is `r (u089n_u122n_kendall$estimate) %>% round(digits = 3)` indicating that the two variables share only `r ((u089n_u122n_kendall$estimate ^ 2) * 100) %>% round(digits = 1)`% of variance.
...
The test is significant (*p* < .01), thus we can say that there is an inverse relationship, which is however very weak. The value for tau is `r (u118n_u120u122n_kendall$estimate) %>% round(digits = 3)` indicating that the two variables share only `r ((u118n_u120u122n_kendall$estimate ^ 2) * 100) %>% round(digits = 1)`% of variance.
