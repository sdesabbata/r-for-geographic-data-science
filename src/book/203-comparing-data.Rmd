...
- *t*(`r t_test__ver_and_vir %$% parameter[["df"]] %>% round(digits = 2)`) = `r t_test__ver_and_vir %$% statistic[["t"]] %>% round(digits = 2)`, *p* `r t_test__ver_and_vir %$% p.value %>% {if_else(. < 0.01, "< 0.01", as.character(round(., digits = 2)))}`
...
The difference is significant F(`r iris_anova_summary[[1]] %$% Df[1] %>% round(digits = 2)`, `r iris_anova_summary[[1]] %$% Df[2] %>% round(digits = 2)`) = `r iris_anova_summary[[1]][1, 4] %>% round(digits = 2)`, *p* < .01. Also, note how the the output above contains a row for the `Residuals` with a related value for the column `Sum Sq`, which is the SSR value for the simple model used to calculate the ANOVA statistics.
...
library(sf)
leic_2011OAC_shp <- 
  read_sf("tmp/E06000016/E06000016_Leicester.shp")
...
flats_and_cars %>%
  ggplot(aes(x = perc_flats, y = perc_2ormore_cars)) +
  geom_point(aes(color = supgrpname)) +
  scale_color_manual(values = c("#377eb8", "#e41a1c", "#f781bf", "#ffff33", "#ff7f00", "#984ea3", "#a65628")) +
  theme_bw()
```
