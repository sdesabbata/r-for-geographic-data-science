...
To summarise, abstract algorithms are implemented as programs using variables and one or more functions.
...
As mentioned above, libraries are collections of functions and/or datasets. Libraries can be installed in `R` using the function `install.packages` or using `Tool > Install Packages...` in RStudio. 
